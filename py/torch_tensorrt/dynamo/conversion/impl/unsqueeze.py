...
def unsqueeze_old(
  ...
  new_shape = list(tuple(input_val.shape)[:intermediate_dim] + (1,) + tuple(input_val.shape)[intermediate_dim:])
...
def broadcast_in_dim(
  ...
  assert all(dim in (1, None) for dim in augmented_shape_list), (
